		...
		if(data.i[0]*1){
			...
			if(data.i.length > 1){
				img.onclick = function(e){
					if((e.x - img.x) / img.width > .5){
						// forward
						if(index+1 < data.i.length){
							index++;
							img.src = '?pc='+data.i[index];
						}
					} else {
						// backward
						if(index > 0){
...
var main = [1].map(x => document.getElementById('main'+x));
main[0].style.width = '60%';
main[0].style.marginLeft = '20%';
[2,3].forEach(x => document.getElementById('main'+x).style.width = '0');
...
function analyse(x, getMain){
	...
	x.split(';').forEach(x => {
		...
		case '1':
			...
			thread = createContent(getMain(), {c:creator,t:content,i:images,l:likes,p:party,dc:dtCreated,du:dtUpdated});
			break;
...
var data = document.getElementsByTagName('data')[0].innerText;
analyse(data, getMain);
